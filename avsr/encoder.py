...
class AttentiveEncoder_test(Seq2SeqEncoder):
    ...
    def __init__(self,
        ...
        super(AttentiveEncoder_test, self).__init__(
            data,
            mode,
            hparams,
            num_units_per_layer,
...
#                self._middle_encoder_outputs, self._middle_encoder_final_state = tf.nn.dynamic_rnn(
#                    cell=MultiRNNCell(self._encoder_cells[0:-1]),
#                    inputs=encoder_inputs,
#                    sequence_length=self._inputs_len,
#                    parallel_iterations=self._hparams.batch_size[0 if self._mode == 'train' else 1],
#                    swap_memory=False,
...
#
#                self._encoder_outputs, self._encoder_final_state = tf.nn.dynamic_rnn(
#                    cell=attention_cells,
#                    inputs=self._middle_encoder_outputs,
#                    sequence_length=self._inputs_len,
#                    parallel_iterations=self._hparams.batch_size[0 if self._mode == 'train' else 1],
#                    swap_memory=False,
#                    dtype=self._hparams.dtype,
...
#                    )
                ...
                self._encoder_middle_outputs, self._encoder_middle_state = tf.nn.dynamic_rnn(
                    cell=MultiRNNCell(self._encoder_cells[0:-1]),
                    inputs=encoder_inputs,
                    sequence_length=self._inputs_len,
                    parallel_iterations=self._hparams.batch_size[0 if self._mode == 'train' else 1],
                ...
                self._encoder_outputs, self._encoder_final_state = tf.nn.dynamic_rnn(
                    cell=self._encoder_cells[-1],
                    inputs=self._encoder_middle_outputs,
                    sequence_length=self._inputs_len,
                    parallel_iterations=self._hparams.batch_size[0 if self._mode == 'train' else 1],
                    swap_memory=False,
                    dtype=self._hparams.dtype,
                ...
                if self._hparams.write_attention_alignment is True:
                    self.attention_summary = self._create_attention_alignments_summary_test(self._encoder_final_state)

    def _create_attention_alignments_summary_test(self, states):
        r"""
        Generates the alignment images, useful for visualisation/debugging purposes
        """
        attention_alignment = states.alignment_history.stack()
    ...
    def get_data(self):
        ...
        return EncoderData(
            outputs=self._encoder_outputs,
            final_state=(self._encoder_middle_state, self._encoder_final_state.cell_state)
        )
